...
export class PacienteEgresoService {
  ...
  static readonly URL: string = "admision";
    ...
    listaCompleta(pagina:number = 1, resultados_por_pagina:number =20 ): Observable<any>{
      return this.jwtRequest.get("paciente",null,{page: pagina, per_page: resultados_por_pagina}).map( (response: Response) => response.json().data);
    }

    buscarCompleta(term: string, pagina:number = 1, resultados_por_pagina:number =20 ): Observable<any>{
      return this.jwtRequest.get("paciente",null,{q: term, page: pagina, per_page: resultados_por_pagina}).map( (response: Response) => response.json().data);
    }
   ...
   editar(id:any, pacienteEgreso: PacienteEgreso): Observable<PacienteEgreso> {
     return this.jwtRequest.put("paciente-egreso",id, pacienteEgreso).map( (response: Response) => response.json().data) as Observable<PacienteEgreso>;
   }
   ...
   crear(pacienteEgreso: PacienteEgreso): Observable<PacienteEgreso> {
      return this.jwtRequest.post("paciente",pacienteEgreso).map( (response: Response) => response.json().data) as Observable<PacienteEgreso>;
    }
