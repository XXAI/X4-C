...
export class PacienteService {
	...
	static readonly URL: string = "paciente";
  	...
  	listaMunicipios( ): Observable<any>{
	    return this.jwtRequest.get("municipio",null,null).map( (response: Response) => response.json().data);
	  }

  	listaLocalidades(municipio: number): Observable<any>{
  	    return this.jwtRequest.get("localidad",null,{municipio_id: municipio}).map( (response: Response) => response.json().data);
  	}
   ...
   historial(id:any, paciente: Paciente): Observable<Paciente> {
     return this.jwtRequest.get("historial",id, null).map( (response: Response) => response.json().data) as Observable<Paciente>;
   }
